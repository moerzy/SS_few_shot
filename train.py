...
def main_worker(argss):
    ...
    if args.model == 'pfenet':
        ...
        raise ValueError(f'do not support model {args.model}')#raise手动抛出异常
...
def train(train_loader, val_loader, model, optimizer, epoch, max_iou, save_path):
    ...
    main_loss_meter = AverageMeter()#AverageMeter()可以记录当前的输出，累加到某个变量之中，然后根据需要可以打印出历史上的平均
