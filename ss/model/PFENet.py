...
class PFENet(nn.Module):
    ...
    def __init__(self, backbone, self_supervision, prior):
        ...
        self.pyramid_bins = [60, 30, 15, 8]#4个不同的平均池化核的大小
    ...
    def init_fem(self, reduce_dim=256):
        ...
        Returns:
            initialised layers
        """
        init_merge = nn.ModuleList()#ModuleList它可以以列表的形式来保持多个子模块
    ...
    def get_corr_mask(x, y):
        ...
        # choose the highes similarity score for each query pixel选取每块中最大值的那个相似度得分
    ...
    def forward_FEM(self, query_feat, prototype, prior):
        ...
        out = self.cls(query_feat) #cls是分类头函数
        return out, out_list
...
#？？？？？？？？？？？？？？？？？？？？？？？？？？
    ...
    def forward(self, input_dict):
        ...
        y = input_dict['q_y']  # (B, H, W) 查询图像的掩码
        s_x = input_dict['s_x']  # (B, S, 3, H, W)
        s_y = input_dict['s_y']  # (B, S, H, W) 支持图像的掩码

        b, shot, _, H, W = s_x.shape
        s_x = s_x.reshape(-1, 3, H, W)  # (B*shot, 3, h, w)

        query_feat_1, query_feat_2, query_feat_3, query_feat_4 = self.forward_backbone(x) # (B, C, h, w) 4个block得到的查询集特征
        query_feat = torch.cat([query_feat_3, query_feat_2], dim=1)  # (B, C, h, w) 查询集的特征

        # supervised pathway
        supp_feat_1, supp_feat_2, supp_feat_3, supp_feat_4 = self.forward_backbone(s_x)  # (B*shot, C, h, w)  4个block得到的支持集的特征
        supp_feat = torch.cat([supp_feat_3, supp_feat_2], dim=1)  # (B*shot, C, h, w)  支持集的特征
        out, out_list = self.forward_comparison(
            query_feat, query_feat_4,
            supp_feat, supp_feat_4, s_y
        )
        out = F.interpolate(out, size=(H, W), mode='bilinear', align_corners=True)
        out_binary = out.argmax(dim=1)#这一步是为什么，难不成是因为前一步做了上采样
        ...
        if self.training:
            ...
            out_dict['main_loss'] = main_loss   #有监督的损失
            out_dict['aux_loss'] = aux_loss    #有监督的损失
